...
echo "${C_GREEN}${C_BOLD}🌍 EcoPulse — Backend Setup${C_RESET}"
...
missing=()
for required in server.py climate_agent.py climate_llm.py weather.py cache.py requirements.txt env.example Modelfile.ecopulse region_config.py; do
...
if ! $REGION_EXPLICIT && ! $NON_INTERACTIVE; then
  available_regions=()
  for cfg in "$BOT_DIR"/configs/*.json; do
    [[ -f "$cfg" ]] && available_regions+=("$(basename "${cfg%.json}")")
  done
  if [[ ${#available_regions[@]} -gt 0 ]]; then
    echo "Available regions: ${available_regions[*]}"
    read -r -p "Which region should this deployment use? [$REGION]: " region_input
...
ok "Core third-party packages import cleanly"

log "Verifying every EcoPulse module actually imports (this is what catches a missing file like cache.py — climate_agent.py imports it lazily inside a function, so a bare 'import climate_agent' wouldn't otherwise notice it's gone)"
project_import_result="$(cd "$BOT_DIR" && ECOPULSE_REGION="$REGION" "$RUN_PYTHON" -c "
import sys, traceback
sys.path.insert(0, '.')
modules = ['region_config', 'climate_llm', 'weather', 'cache', 'climate_agent']
for m in modules:
    try:
        __import__(m)
    except Exception:
...
" 2>&1)"
if [[ "$project_import_result" == *"FAIL|"* ]]; then
  echo "$project_import_result" >&2
  fail "A project module failed to import — see traceback above. Every .py file the codebase depends on needs to be present in $BOT_DIR."
fi
ok "All EcoPulse modules import cleanly: $(echo "$project_import_result" | grep '^OK|' | cut -d'|' -f2)"
...
if [[ -f "$ENV_PATH" ]] && ! $FORCE_ENV; then
  ...
  # Ask for a Gemini fallback key if one wasn't passed via --gemini-key.
  # Without it, GEMINI_API_KEY just stays blank and the app falls back to
  # the offline keyword-based reply when Ollama isn't reachable — silently,
  # with no indication this was even an option. Worth asking once, up front.
  if [[ -z "$GEMINI_KEY" ]] && ! $NON_INTERACTIVE; then
    echo
    echo "  Gemini API key (optional) — used as a cloud fallback whenever Ollama"
    echo "  isn't reachable. Free key: https://aistudio.google.com/apikey"
    read -r -p "  Paste a Gemini API key, or press Enter to skip: " gemini_input
    [[ -n "$gemini_input" ]] && GEMINI_KEY="$gemini_input"
  fi

  # Admin token: default is to auto-generate a real random one instead of
  # shipping env.example's placeholder ("my_secure_demo_secret_token_123") —
  # that placeholder being live in a real deployment was a flagged backlog
  # item. Only ask if the person wants to supply their own instead.
  if [[ -z "$ADMIN_TOKEN" ]]; then
    if command -v openssl >/dev/null 2>&1; then
      generated_token="$(openssl rand -hex 16)"
    else
      generated_token="$("$RUN_PYTHON" -c 'import secrets; print(secrets.token_hex(16))')"
    fi
    if ! $NON_INTERACTIVE; then
      echo
      echo "  Admin token — required to unlock status-change actions on the org"
      echo "  dashboard (/dashboard). A secure one has been generated for you."
      read -r -p "  Press Enter to use it, or type your own token: " token_input
      ADMIN_TOKEN="${token_input:-$generated_token}"
    else
  ...
  if [[ -z "$GEMINI_KEY" ]]; then
    echo "    Gemini fallback: not set — fine if Ollama is running, otherwise"
    echo "    the app falls back to canned offline replies. Add GEMINI_API_KEY"
    echo "    to .env any time, or re-run with --force-env --gemini-key <key>."
  fi
fi
...
if $SKIP_BOOT_TEST; then
  ...
  if port_in_use "$SERVER_PORT"; then
    ...
    DASH_CODE="$(curl -sL -o /dev/null -w '%{http_code}' "http://localhost:$SERVER_PORT/dashboard")"
...
# ---------------------------------------------------------------------------
# Done — hand off to a real, persistent server run (this is the "start"
# half of "installation and start script"). The boot test above already
# proved the server works; this is the actual long-running instance.
# ---------------------------------------------------------------------------
...
if $NO_START; then
  echo "Next steps (--no-start was passed, so nothing is running right now):"
  if $NO_VENV; then
  ...
  echo "  Org dashboard  : http://localhost:$SERVER_PORT/dashboard"
  echo "  Android emulator points at http://10.0.2.2:$SERVER_PORT automatically"
  exit 0
fi

if port_in_use "$SERVER_PORT"; then
  warn "Port $SERVER_PORT is already occupied (likely by the boot-test instance failing to shut down cleanly, or something else entirely)."
  fail "Free up port $SERVER_PORT and re-run, or run with --no-start and start the server yourself once the port is clear."
fi

log "Starting the EcoPulse server for real — this is now the running instance"
echo "  Org dashboard  : http://localhost:$SERVER_PORT/dashboard"
echo "  Android emulator points at http://10.0.2.2:$SERVER_PORT automatically"
echo "  Press Ctrl+C to stop."
echo
trap - EXIT   # the boot-test cleanup trap no longer applies — this process IS the server now
cd "$BOT_DIR"
exec "$RUN_PYTHON" server.py